...
class DeepseekV4Indexer(nn.Module):
    ...
    def select_qli(
        ...
        if use_v2:
            # v2 kernel contract (ops-transformer arch35
            # quant_lightning_indexer_v2_kernel: GetActualSeqLenKey):
            #   actS2SizeOrig = seqused_k * cmp_ratio + cmp_residual_k
            #   actS2Size = actS2SizeOrig / cmp_ratio
            # so seqused_k is the COMMITTED compressed-key count
            # (raw_kv_len // cmp_ratio) and cmp_residual_k is
            # raw_kv_len % cmp_ratio. seqused_q is per-sequence query
            ...
            topk, _ = kernels.quant_lightning_indexer_v2(
                ...
                q_scale,
                key_dequant_scale,
                cu_seq_lens_q, None,  # cu_seqlens_q (B+1, leading 0), cu_seqlens_k
                seqused_q, seqused_k,  # seqused_q (per-seq), seqused_k (compressed)
                cmp_residual_k,  # cmp_residual_k (raw % cmp_ratio)
                block_table,  # block_table
                ...
                "TND", "PA_BBND",  # layout_q, layout_k (v2 kernel only
                # accepts PA_BBND/BSND/TND for the paged key; PA_BSND is
                # rejected by the tiling function)
